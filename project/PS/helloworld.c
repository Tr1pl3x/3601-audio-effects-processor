...
int new_state(uint32_t current_state) {
...
int button_state_prev = 0;
 ...
 while(1) {
    ...
    xil_printf("going to dma\n");
    dma_read();
    xil_printf("leaving the dma\n");
    //updated the buffer size
    buf_stored_size += BUF_SIZE * SAMPLES_READ_AT_ONCE;
    //if the buffer is filled
    if (buf_stored_size == BUF_SIZE) {
    ...
    if ((button_state != button_state_prev) && (button_state == 0)) {
        ...
        xil_printf("new state = %d\n", state);
        Xil_Out32(XPAR_AUDIO_PIPELINE_0_BASEADDR + AXI_AUDIO_SEL_OFFSET, state);
        if (state == WRITE_FILE) {
        	sd_write(rx_buf, buf_stored_size, full_filename);
        	buf_stored_size = 0;
        	file_written_count += 1;
