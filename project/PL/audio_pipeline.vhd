...
architecture Behavioural of audio_pipeline is
    ...
    signal sig_fifo_data_w, sample  : std_logic_vector(DATA_WIDTH-1 downto 0);
    ...
    signal sig_effect_sel           : std_logic_vector(2 downto 0); --fsm state

    --------------------------------------------------
    -- Audio effects signals
    --------------------------------------------------
    signal sample_ready              : std_logic;
    signal sig_speed_reg             : std_logic_vector(31 downto 0);
...
begin
	...
	port map (
		...
		S_AXI_WDATA	    => s00_axi_wdata, --used to send data from ps to pl
    ...
    port map (
        ...
        fifo_din        => sample,--sig_fifo_data_w,
        fifo_w_stb      => sample_ready, --sig_fifo_wr,
    ...
    --sig_fifo_data_w <= sample; --sample;
    --sig_fifo_wr <= sample_ready;
    ...
    --FLANGER
    --------------------------------------------------
    audio_effects_inst: entity work.audio_effects
    generic map (
        DATA_WIDTH => 32,
        PCM_PRECISION => 18,
        GAIN_WIDTH  => 32,
    ...
    port map (
        clk => clk,
        rst => rst,
        -- Input audio stream from the i2s mic
        audio_in => sample,
        valid_in => sample_ready,
        ...
        audio_out  => sig_fifo_data_w,
        valid_out  => sig_fifo_wr
    );
