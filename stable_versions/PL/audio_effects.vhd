...
entity audio_effects is
    ...
    port (
        ...
        echo_delay_reg   : in  std_logic_vector(31 downto 0);  -- Delay time in samples
        echo_feedback_reg: in  std_logic_vector(31 downto 0);  -- Feedback gain (Q16.16)
        echo_mix_reg     : in  std_logic_vector(31 downto 0);  -- Wet/dry mix (Q16.16)
...
architecture Behavioral of audio_effects is
    ...
    signal gain_stage4 : std_logic_vector(DATA_WIDTH - 1 downto 0);
    signal gain_valid_stage4 : std_logic;
    signal echo_stage4      : std_logic_vector(DATA_WIDTH - 1 downto 0);
    signal echo_valid_stage4 : std_logic;
    signal speed_stage2, speed_stage3, speed_stage4 : std_logic_vector(DATA_WIDTH - 1 downto 0);
    signal speed_valid_stage2, speed_valid_stage3, speed_valid_stage4 : std_logic;
...
begin
    ...
    ----------------------------------------------------------------------------
    -- Bypass Path (None) - 4-stage pipeline to match flanger effect latency
    ----------------------------------------------------------------------------
    ...
    begin
        if rising_edge(clk) then
            if rst = '1' then
                ...
                speed_stage2 <= speed_out;
                speed_valid_stage2 <= speed_valid;

                -- Stage 3
                bypass_stage3 <= bypass_stage2;
                bypass_valid_stage3 <= bypass_valid_stage2;
                speed_stage3 <= speed_stage2;
                speed_valid_stage3 <= speed_valid_stage2;
                
                --Stage 4
                bypass_stage4 <= bypass_stage3;
                bypass_valid_stage4 <= bypass_valid_stage3;
                --match latency of echo and gain (3) and speedup (1) to latency of flanger(4)
                gain_stage4 <= gain_out;
                gain_valid_stage4 <= gain_valid;
                echo_stage4 <= echo_out;
                echo_valid_stage4 <= echo_valid;
                speed_stage4 <= speed_stage3;
                speed_valid_stage4 <= speed_valid_stage3;
    ...
    ----------------------------------------------------------------------------
    -- Effect 1: Echo (Implemented with delay, feedback, and wet/dry mix)
    ----------------------------------------------------------------------------
    echo_effect_inst : echo_effect
        generic map (
            DATA_WIDTH => DATA_WIDTH,
            PCM_PRECISION => PCM_PRECISION,
            MAX_DELAY_SAMPLES => 12000  -- 250ms at 48kHz
        )
        port map (
            clk => clk,
            rst => rst,
            audio_in => audio_in,
            valid_in => valid_in,
            delay_reg => echo_delay_reg,
            feedback_reg => echo_feedback_reg,
            mix_reg => echo_mix_reg,
            audio_out => echo_out,
    ...
    ----------------------------------------------------------------------------
    clipper_effect_inst : entity work.clipper_effect
        generic map (
            DATA_WIDTH    => DATA_WIDTH,
            PCM_PRECISION => PCM_PRECISION
        )
    ...
    begin
        case effect_selector is
            ...
            when "001" =>  -- Echo
                audio_out <= echo_stage4;
                valid_out <= echo_valid_stage4;

            when "010" =>  -- Gain
                audio_out <= gain_stage4; --gain_out;
                valid_out <= gain_valid_stage4;

            when "011" =>  -- Speed up/Slow down
                audio_out <= speed_stage4;
                valid_out <= speed_valid_stage4;
